...
def run_migrations(deploy_path=DEPLOY_PATH):
    with cd(deploy_path):
        with prefix("source {venv}/bin/activate".format(venv=VENV_DIR)):
            sudo(
                "CABOT_CONFIG=conf/{env}.env foreman run python manage.py syncdb -e conf/{env}.env".format(env=env.deploy_version))
            sudo(
                "CABOT_CONFIG=conf/{env}.env foreman run python manage.py migrate cabotapp --noinput -e conf/{env}.env".format(env=env.deploy_version))
            ...
            with settings(warn_only=True):
                sudo(
                    "CABOT_CONFIG=conf/{env}.env foreman run python manage.py migrate djcelery --noinput -e conf/{env}.env".format(env=env.deploy_version))
...
def collect_static(deploy_path=DEPLOY_PATH):
    with cd(deploy_path):
        with prefix("source {venv}/bin/activate".format(venv=VENV_DIR)):
            sudo(
                "CABOT_CONFIG=conf/{env}.env foreman run python manage.py collectstatic --noinput -e conf/{env}.env".format(env=env.deploy_version))
            sudo(
                "CABOT_CONFIG=conf/{env}.env foreman run python manage.py compress -e conf/{env}.env".format(env=env.deploy_version))
